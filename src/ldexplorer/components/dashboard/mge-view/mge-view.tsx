...
@Element() element: HTMLElement;
  ...
  async _showChart(node, parentId, typeChart, isFromEdge=false, secondNode=null, isFromCluster=false, isFromHC=false, newQuery=null) {
          ...
          console.log("isFromEdge: " + isFromEdge);
          console.log("secondNode: " + secondNode);
          console.log("isFromCluster: " + isFromCluster);
          console.log("isFromHC: " + isFromHC);
    ...
    console.log(this.datasetName);
          ...
          console.log("state._data[this.datasetName]");
          console.log(state._data[this.datasetName]);
  ...
  @Method()
  async _showChart2(node, parentId, typeChart, isFromEdge = false, secondNode = null, isFromCluster = false, isFromHC = false, newQuery = null) {
    let title, chartNode, viewChild, link, convertData;
    console.log("------------------------");
    ...
    console.log(chartNode);
    console.log("first");
    console.log("data-" + (state.indexQueryData - 1));
    console.log("state._data[this.datasetName]");
    console.log(state._data["data-" + state.indexQueryData]);
    
    
    await chartNode.setData(node, state._data["data-" + state.indexQueryData], secondNode, isFromEdge, isFromCluster, isFromHC)
    console.log("second");
    convertData = await chartNode.setData()
    //console.log("convertData: "+JSON.stringify(convertData));
    state.savedData = convertData;
    ...
    if (newQuery)
      ...
      title = await this.generateTitle(node, convertData, typeChart, parentId, isFromEdge, secondNode, isFromCluster, isFromHC)
    await viewChild.setTitle(title)
    let viewParent = this._dashboard.shadowRoot.querySelector("mge-view[id-view='" + parentId + "']");
    link = await this._dashboard._addLink(viewParent, viewChild)
    await this._dashboard.addChart(parentId, {
      id: childId, title: title, typeChart: viewChild.typeVis, hidden: false,
      x: viewChild.x, y: viewChild.y, view: viewChild, link: link
    })
