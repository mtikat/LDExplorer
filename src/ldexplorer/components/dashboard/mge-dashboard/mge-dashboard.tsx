...
async closeView(view) {
  ...
  async buildDashboard(_svg, query, data){
    ...
    for (let index = 1; index < data.data.length; index++) {
      ...
      if (element.typeChart == 'annotation'){
        ...
        promises.push(await query.node()._view._showChart2(element.node, element.parentId, element.typeChart, element.isFromEdge, element.secondNode, element.isFromCluster, element.isFromHC, element.newQuery))
      }
  ...
  async loadAnnotation(_svg){
    let id_annotation = this.urlParams.get("id-annotation");
    let data= null
    console.log(id_annotation)
    if (id_annotation != undefined && id_annotation != null ) {
      await fetch(window.location.protocol + '//' + window.location.host + '/getAnnotation?id=' + id_annotation, {
        method: 'GET'
      }).then(response => {
        if( response.ok){
          console.log(response.clone().json())
    ...
    this._addLinkAnnotation(parents,this._annotationChart.node()).then(async(links) => {
      ...
      for (let i = 0; i <= links.lines.length; i++) {
          ...
          await this.addChart(id_parent, {
              id: this._annotationChart.node().idView, title: idAnnotation + " by " + element.data.user, typeChart: this._annotationChart.node().typeVis, hidden: false, x: this._annotationChart.node().x, y: this._annotationChart.node().y, view: this._annotationChart.node(), link: { line: line, conect: links.conect, visible: links.visible }
          })
