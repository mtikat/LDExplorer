    ...
    */
    ...
    async setData(_) {
        ...
        this._vNodes.forEach((n, i) => {
            ...
            if (n.data.typeChart != "mge-query"){
                //n.data.typeChart != "mge-annotation" && 
                state._annotationdata.push({
                    "title": n["data"].title,
                    "typeChart": n["data"].typeChart
                });
        ...
        if (_.length > 1) {
            for (let index = 1; index < _.length; index++) {
                ...
                n["y"] = 0;
                state._historydata.push(n["data"].title);
                // state._annotationdata.push({"title": n["data"].title,
                //     "typeChart": n["data"].title});
                this._vNodes.push(n);
            }
        }
        // _irisPanel.update();
