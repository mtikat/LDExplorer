...
export class MgeListing {
  ...
  sortByFirstAuthor = function () {
    this._sortByText = false;
    this._data.root.data.documents.sort((x, y) => {
      if (x.authors[0] == this._data.root.data.id || y.authors[0] == this._data.root.data.id) return -1;
      else {
  ...
  getValue(value, data, title) {
    ...
    value['data']['objtype'] = [data.type.label];
  ...
  addPaperListChart(idDiv, divTag) {
    ...
    this.model.when('box', box => {
      ...
      select(this.element.querySelectorAll('.paper-list')[0])
    ...
    //---------------------
    this.model.when(
      ['data', 'widthChart', 'heightChart', 'redraw'],
      () => {
        if (this._grpPapers !== null) this._grpPapers.remove();
        this._data = this.model.data;

        if (this._sortByText) this.sortByText();
        ...
        if (this.model.data.children.cluster === true) {
          if (this._names !== null) this._names.remove();
          ...
          let y = 12;
          this._names = this._grpPapersList
            .selectAll('.PL-grpNames')
            .data(authors)
            .enter()
            .append('text')
            .attr('class', 'PL-names')
        ...
        let x = 5,
          y = this.model.data.children.cluster === true ? endOfNames + 15 : 15;

        this._grpPapers = this._grpPapersList
          .selectAll('.PL-grpPapers')
        ...
        this._grpPapers
          ...
          .attr('class', 'PL-infos')
          .text(d => {
            d.authorList = this._getAuthorList(d);
            return this._getAuthorList(d);
          })
        ...
        function updateSVGWidth() {
          globalThis._grpPapers.selectAll('text').each(function () {
