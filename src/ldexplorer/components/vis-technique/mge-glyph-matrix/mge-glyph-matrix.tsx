...
export class MgeGlyphMatrix {
  ...
  private _legend = {
    // Barra onde est�o os textos como os nomes dos nodos
  ...
  async addMatrixGlyph(idDiv, divTag) {
    ...
    (this._tooltips.divTT = divTag.append('div').style('display', 'none').classed('MG-Tooltip', true)), // Tooltip para o nodo normal
      (this._dragListenerL = drag().on('drag', this._chartDragLegend.bind(this)).on('end', this._chartDragendLegend.bind(this)));

    this._dragListenerM = drag().on('drag', this._chartDragMatrix.bind(this)).on('end', this._chartDragendMatrix.bind(this));
    ...
    this._grpTopLegend = this._grpMatrixGlyph
      .append('g') // Grupo da legenda superior
      .attr('class', 'MG-TopLegend')
      .attr('clip-path', 'url(#' + this._idClipTop + ')');
    this._grpDragTL = this._grpTopLegend
      .selectAll('g')
      .data([{ x: 0, y: 0, cellsInv: 0, t: 0 }])
      .enter()
      .append('g')
      .call(this._dragListenerL);

    this._grpLeftLegend = this._grpMatrixGlyph
      .append('g') // Grupo da legenda esquerda
      .attr('class', 'MG-LeftLegend')
      .attr('clip-path', 'url(#' + this._idClipLeft + ')');
    this._grpDragLL = this._grpLeftLegend
      .selectAll('g')
    ...
    this._grpMatrix = this._grpMatrixGlyph
      .append('g') // Grupo da matriz
      .attr('class', 'MG-Matrix')
      .attr('clip-path', 'url(#' + this._idClipMatrix + ')');
    this._grpDragMT = this._grpMatrix
      .selectAll('g')
      .data([{ x: 0, y: 0, cellsInvX: 0, cellsInvY: 0 }])
      .enter()
      .append('g')
      .call(this._dragListenerM);
    ...
    this.model.when(['box', 'margin'], (box, margin) => {
      (this.model.widthChart = box.width - margin.left - margin.right), (this.model.heightChart = box.height - margin.top - margin.bottom);
    });
    ...
    this.model.when(['data', 'redraw'], async (data, widthChart, heightChart, redraw) => {
      ...
      this._cellCoordScale.range([0, (this._matrixWidth * this._lines.qtTotal) / this._lines.qtVisible]).domain(this._vOrder);
      ...
      this._overview.rectBk.attr('width', this._legend.totalWidth - 1).attr('height', this._legend.totalWidth - 1);
  ...
  _appendLeftLegend(data) {
    if (this._grpBarsLL != null) this._grpBarsLL.remove();

    this._grpBarsLL = this._grpDragLL
      .selectAll('g')
      .data(data.nodes.dataNodes)
      .enter()
      .append('g')
      .attr('transform', (d, i) => {
        return 'translate(0,' + this._cellCoordScale(i) + ')';
      });

    this._grpBarsLL.append('rect').attr('class', 'GM-node').attr('width', this._legend.width).attr('height', this._cell.height).style('fill', this._colors.legendBk);
    ...
    this._grpBarsLL
      ...
      .text((d, i) => {
        return this._adjustLengthText(d.labels[1], 12);
      })
      .append('title')
      .text(d => d.labels[1]);
  }
  ...
  _adjustLengthText(stText, limit) {
    if (stText.length > limit) return stText.slice(0, limit) + '...';
    else return stText;
  }

  //---------------------
  _appendTopLegend(data) {
    if (this._grpBarsTL != null) this._grpBarsTL.remove();

    this._grpBarsTL = this._grpDragTL
      .selectAll('g')
      .data(data.nodes.dataNodes)
      .enter()
      .append('g')
    ...
    this._grpBarsTL.append('rect').attr('class', 'GM-node').attr('width', this._legend.width).attr('height', this._cell.height).style('fill', this._colors.legendBk);
  ...
  _appendMatrix(data) {
    var globalThis = this;
    if (this._grpLines != null) this._grpLines.remove();

    this._cellGlyph.calcScale(this._cellCoordScale.bandwidth());
    this._grpLines = this._grpDragMT
      .selectAll('g')
      .data(data.matrix)
      .enter()
      .append('g')
      ...
      .each(async function (d, i) {
        // this � a refer�ncia para cada elemento da sele��o .line
        await drawCells(d, this, globalThis); // Desenha cada c�lula da matriz
      });

    async function drawCells(d, elemThis, globalThis) {
      var cells = select(elemThis)
        .selectAll('.MG-Cell')
        .data(
          d.filter(function (d) {
            return d.exist;
          }),
        ) // Seleciona as c�lulas que possuem dados
        .enter()
        .append('g')
        .attr('class', 'MG-Cell')
        .attr('transform', (d, i) => {
          return 'translate(' + globalThis._cellCoordScale(d.x) + ',0)';
        }) // Inclui grupos de cada c�lula da linha
      ...
      cells
        ...
        .style('fill', function (d) {
          ...
          } else return globalThis._cellColorMap.fMap(d.labels[globalThis._cellColorMap.indexAttrColor]);
        })
  ...
  async _onMouseEnterNode(event, d) {
    ...
    if (this._tooltips.matrixCell != null) this._tooltips.matrixCell.create(this._tooltips.divTT, d, event);
  }
  ...
  _onMouseClick(d, data) {
    ...
    Object.entries(state.annotations).forEach(([key, value]) => {
      ...
      if (value['disabled'] == false) {
        state.annotations[key] = this.getValue(value, data, title);
      }
    });
  ...
  async _onMouseLeaveNode(event, d) {
    if (this._tooltips.matrixCell != null) this._tooltips.matrixCell.remove();
  }
  ...
  async _calcVisibleLines() {
    ...
    if (this._lines.qtMaxVisible > 3) this._lines.qtMinVisible = 3;
    else this._lines.qtMinVisible = 1;

    if (this._lines.qtMaxVisible > this._lines.qtTotal) this._lines.qtMaxVisible = this._lines.qtTotal;

    if (this._lines.qtMaxVisible < this._lines.qtVisible && this._lines.qtMaxVisible != 0) this._lines.qtVisible = this._lines.qtMaxVisible;
  }
  ...
  async _calcHeightCell() {
    ...
    if (h > 2) h -= 1; // Ajusta para margem de 1 px entre c�lulas
    return h;
  }
  ...
  async _limCoord(coord) {
    if (coord < this._legend.coordMin) return this._legend.coordMin;
    else if (coord > 0) return 0;
    else return coord;
  }
  ...
  @Method()
  async setBox(_) {
    if (!arguments.length) return this.model.box;
    this.model.box = _;
  }
  ...
  @Method()
  async setPanel(_) {
    if (!arguments.length) return this._matrixGlyphPanel;
  ...
  async setData(_, globalData) {
    var qtLabel = 0,
      qtValue = 0;
    if (!arguments.length) return this.model.data;
    if (_.cluster) {
    ...
    this.model.data = _;
    if (this.model.data.nodes.labelTitle != null) qtLabel = this.model.data.nodes.labelTitle.length;

    if (this.model.data.nodes.valueTitle != null) qtValue = this.model.data.nodes.valueTitle.length;
  ...
  async indexAttrSort(_) {
    if (!arguments.length) return this._indexAttrSort;
  ...
  async indexAttrLegend(_) {
    if (!arguments.length) return this._indexAttrLegend;
  ...
  async indexAttrCellColor(_) {
    if (!arguments.length) return this._cellColorMap.indexAttrColor;
  ...
  async pLegendWidth(_) {
    if (!arguments.length) return this.model.pLegendWidth;
  ...
  async cellColorsMap(colors) {
    ...
    } else if (typeof colors === 'function') this._cellColorMap.fMap = colors;
    else {
  ...
  @Method()
  async pFontHeight(_) {
    if (!arguments.length) return this.model.pFontHeight;
  ...
  @Method()
  async glyph(_) {
    if (!arguments.length) return this._cellGlyph;
    this._cellGlyph = _;
  }
  /*  
      //---------------------   
  ...
  */
  ...
  async acSortExec(_) {
    ...
    this._grpLines
      ...
      .selectAll('.MG-Cell')
      .attr('transform', (d, i) => {
        return 'translate(' + this._cellCoordScale(d.x) + ',0)';
      });
  }
  ...
  async acChangeAttrLegend(_) {
    ...
    this._grpBarsLL.selectAll('text').text((d, i) => {
      return d.labels[this._indexAttrLegend];
    });

    this._grpBarsTL.selectAll('text').text((d, i) => {
      return d.labels[this._indexAttrLegend];
    });
  }
  ...
  async acChangeVisibleLines(qtLines) {
    ...
    let calcCellsInv = (dLines, cellsInv) => {
      if (dLines > this._lines.qtTotal - this._lines.qtVisible + cellsInv) return qtLines - this._lines.qtTotal; // cellsInv possui valor negativo
      return cellsInv;
    };
  ...
  componentWillLoad() {}
  componentDidLoad() {
    let svg = select(this.element.querySelectorAll('.glyph-matrix')[0]).attr('width', this.width).attr('height', this.height);
    // console.log(this.id)
