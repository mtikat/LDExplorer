...
export class MgeClustervis {
  ...
  async addClusterChart(idDiv, divTag) {
    ...
    this.model.pInnerRadius = 0.2; // Percentage of the width of the graph for calculating the _innerRadius
    ...
    this._drawLine = radialLine()
      .curve(curveBundle.beta(0.85)) // Generator of splines that makes up the edges
      .radius(function (d) {
        return d.data.y;
      })
      .angle(function (d) {
        return (d.data.x / 180) * Math.PI;
      });
    ...
    var _appendRings = () => {
      if (this._grpRings != null) this._grpRings.remove();
      this._grpRings = this._grpCluster
        .selectAll('.CV-grpRings')
        .data(this._vRings) // Original _vRings
        .enter()
        .append('g')
        .attr('class', 'CV-grpRings');

      this._grpRings.append('circle').attr('r', d => {
        return this._ringScale(d.pHeight);
      });
    };
    ...
    var _appendBars = data => {
      ...
      if (this._grpBars != null) this._grpBars.remove();
      this._grpBars = this._grpCluster
        .selectAll('.CV-grpBars')
        .data(data.nodes.dataNodes)
        .enter()
        .append('g')
        .attr('class', 'CV-grpBars')
        .attr('transform', (d, i) => {
          return 'rotate(' + this._grpBarsRotScale(i) + ')';
        });

      this._grpBars.append('line').attr('x1', this._ringScale(0)).attr('y1', 0).attr('x2', this._ringScale(100)).attr('y2', 0);

      for (i = 0; i < this._vRings.length; i++) {
        if (this._vRings[i].typeAttr === 'L') categoriasAux = categoriasAux.concat(this._vRings[i].vLabelDomain.sort());
      }
      ...
      if (this._sameScale) {
        maxDoAnel = -1;
        for (i = 0; i < this._vRings.length; i++) {
          if (this._vRings[i].maxValue > maxDoAnel) maxDoAnel = this._vRings[i].maxValue;
        }
        for (i = 0; i < this._vRings.length; i++) {
      ...
      for (i = 0; i < this._vRings.length; i++) {
        if (this._vRings[i].typeAttr === 'V') {
          this._grpBars
            ...
            .attr('y', () => {
              return -this._barsArea.widthBar / 2;
            })
            .attr('height', () => {
              return this._barsArea.widthBar;
            })
            .attr('width', d => {
              return this._vRings[i].barCircleScale(d.values[this._vRings[i].indexAttr]);
            })
            .style('fill', () => {
              return this._colorScale(this._vRings[i].indexAttr);
            })
          ...
          this._grpBars
            ...
            .style('fill', d => {
              return this._vRings[i].barCircleScale(d.labels[this._vRings[i].indexAttr]);
            }) //<-- Check how to put the color
            .on('mouseover', this._mouseOverNode.bind(this))
            .on('mouseout', this._mouseOutNode.bind(this))
            .on('click', this._onMouseClick.bind(this))
            .append('title')
     ...
     */
    var _appendLinks = () => {
      if (this._grpLinks != null) this._grpLinks.remove();

      this._grpLinks = this._grpCluster.append('g').attr('class', 'CV-grpLinks').attr('transform', 'rotate(90)');

      this._links = this._grpLinks
        .selectAll('path')
        .data(this._dataLinks.vBundleLinks)
        .enter()
        .append('path')
        .each(function (d) {
          (d.source = d[0].data), (d.target = d[d.length - 1].data);
        })
  ...
  _mouseOverNode(event, d) {
    ...
    this._grpBars.classed('CV-nodeHL', function (node) {
      return node.highLight;
    });

    this._grpBars.append('text').text('').classed('CV-names', true); // For showing names on mouseover
    ...
    function _findMaxValue(_vRings) {
      ...
      for (i = 0; i < _vRings.length; i++) {
        if (_vRings[i].maxValue > max) max = i;
      }
      return max;
    }
  }
  ...
  async _getEdges(dados, nodes) {
    ...
    function findNodo(id) {
      for (let i = 0; i < nodos.length; i++) if (nodos[i].id === id) return nodos[i];
      return null;
    }
    return result;
  }
  ...
  async _updateMaxRings() {
    this._vRings.forEach(ring => {
      if (ring.typeAttr === 'V')
        ring.maxValue = max(this.model.data.nodes.dataNodes, function (d) {
          return d.values[ring.indexAttr];
        });
      else ring.maxValue = 0; // Copy here what was done in the addAttribute()
    });
  }
  ...
  @Method()
  async setPanel(_) {
    if (!arguments.length) return this._clusterVisPanel;
  ...
  async setData(_, globalData) {
    ...
    this._sort.inic(this.model.data.nodes.labelTitle.length, this.model.data.nodes.valueTitle.length).data(this.model.data.nodes.dataNodes);
  ...
  async setpInnerRadius(_) {
    if (!arguments.length) return this.model.pInnerRadius;
  ...
  async setpOuterRadius(_) {
    if (!arguments.length) return this.model.pOuterRadius;
  ...
  removeAnelExterno() {
    ...
    if (this._vRings.length > 0) {
      ...
      for (i = 1; i < this._vRings.length; i++) this._vRings[i].pX = this._vRings[i - 1].pHeight;
    }
  ...
  addAttribute(_indexAttr, _typeAttr) {
    ...
    if (_typeAttr === 'V') {
      maxValue = max(this.model.data.nodes.dataNodes, function (d) {
        return d.values[_indexAttr];
      });
    } else {
      // Determines domain for categorical attributes (deve ser colocado tamb�m na fun��o chart.data)
      maxValue = -1;
      tempKeys = groups(this.model.data.nodes.dataNodes, d => d.labels[_indexAttr]).sort((a, b) => ascending(a[0], b[0]));
      for (i = 0; i < tempKeys.length; i++) _vLabelDomain[i] = tempKeys[i].key;
    }
  ...
  alteraAttribute(_indexAnel, _indexAttr, _typeAttr) {
    ...
    if (_typeAttr === 'V') {
      ...
      // Determines or domain for categorical attributes (should also be placed in the function chart.data)
      maxValue = -1;
      tempKeys = groups(this.model.data.nodes.dataNodes, d => d.labels[_indexAttr]).sort((a, b) => ascending(a[0], b[0]));
    ...
    if (_typeAttr === 'V') this._vRings[_indexAnel].barCircleScale = scaleLinear();
    else this._vRings[_indexAnel].barCircleScale = scaleOrdinal();
    this.model.redraw += 1;
  }
  ...
  async acSortExec(_) {
    ...
    this._grpLinks.selectAll('path').data(this._dataLinks.vBundleLinks).transition().duration(800).attr('d', this._drawLine);
  }
  ...
  acAlteraAnel(indexAnel, indexAttr) {
    ...
    if (indexAtributo >= 1000) this.alteraAttribute(indexAnel, indexAtributo - 1000, 'V');
    else this.alteraAttribute(indexAnel, indexAtributo, 'L');
  ...
  componentDidLoad() {
    let clusterDiv = select(this.element.querySelectorAll('.cluster')[0]).attr('width', this.width).attr('height', this.height);
