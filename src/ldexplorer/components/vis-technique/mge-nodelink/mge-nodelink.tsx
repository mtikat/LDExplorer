...
export class MgeNodelink {
  ...
  private _rNormalNode; // Normal node radius
  private _idLegend;
  ...
  private _configDefault = {
    charge: 100,
    linkDistance: 25, // 30
    fNodoClusterDefault: function () {
      return '';
    },
  };

  private _configLayout;
  ...
  constructor() {
    ...
    (this._forceLayout = forceSimulation()), (this._indexAttrSize = 0); // Just for categorical
  ...
  async addNodeLinkChart(idDiv, divTag) {
    ...
    this._tooltips.divTT = divTag.append('div').style('display', 'none').classed('NE-Tooltip', true); // Tooltip for the normal node
    ...
    this.model.when(['data', 'redraw'], data => {
      ...
      this._configLayout.gravity = 500 / ((Math.PI * this.model.widthChart * this.model.widthChart) / 4 / dataLength);
      if (this._configLayout.gravity < 0.05) this._configLayout.gravity = 0.05;

      if (this._configLayout.gravity < 0.1) this._configLayout.gravity = Math.round(this._configLayout.gravity * 100) / 100;
      else this._configLayout.gravity = Math.round(this._configLayout.gravity * 10) / 10;
  ...
  getValue(value, data, title, type) {
    ...
    let objtype = [];
    if (value['data'].links.length > 0) {
      objtype.push('link');
    }
    if (value['data'].nodes.length > 0) {
      objtype.push('nodes');
    }
    value['data']['objtype'] = objtype;
  ...
  _onMouseClick(type, event, data) {
    ...
    state.viewAnnotation = this.getValue(state.viewAnnotation, data, title, type);

    Object.entries(state.annotations).forEach(([key, value]) => {
      if (value['disabled'] == false) {
        state.annotations[key] = this.getValue(value, data, title, type);
      }
    });
  ...
  _appendNodes() {
    ...
    if (this._graphData.nodes[0].cluster) {
      this._graphElem.nodes = this._grpNodeEdge
        .selectAll('.NE-node')
        ...
        .attr('class', 'NE-node')
        .attr('r', d => {
          return this._rClusterScale(d.qtNodes);
        })
      ...
      this._graphElem.nodes = this._grpNodeEdge
        ...
        .attr('r', d => {
          return this._rClusterScale(d.values[this._indexAttrSize] /*qtNodes*/);
        })
        .style('fill', d => {
          if (d.style != null) {
            return d.style;
  ...
  _appendEdges() {
    if (this._graphElem.edges != null) this._graphElem.edges = null;

    this._graphElem.edges = this._grpNodeEdge
      .selectAll('.NE-edge')
      ...
      .style('stroke-width', d => {
        return this._linkWidthScale(d.qt);
      })
  ...
  _onMouseOverNode(event, d) {
    this._graphElem.nodes.each(function (n) {
      n.highLight = false;
    });
    state.selectedobj = d;
    d.highLight = true;
    if (d.cluster) {
      if (this._tooltips.clusterNode != null) this._tooltips.clusterNode.create(this._tooltips.divTT, d, event);
    } else {
      if (this._tooltips.normalNode != null) this._tooltips.normalNode.create(this._tooltips.divTT, d, event);
    }
    ...
    this._graphElem.nodes.classed('NE-HighLight', function (node) {
  ...
  _onMouseOutNode(event, d) {
    ...
    d.highLight = false;
    if (d.cluster) {
      if (this._tooltips.clusterNode != null) this._tooltips.clusterNode.remove();
    } else {
      if (this._tooltips.normalNode != null) this._tooltips.normalNode.remove();
    }
  }
  ...
  _onMouseOverEdge(event, d) {
    ...
    if (d.source.cluster && d.target.cluster) {
      if (this._tooltips.clusterEdge != null) this._tooltips.clusterEdge.create(this._tooltips.divTT, d, event);
    } else {
      if (this._tooltips.normalEdge != null) this._tooltips.normalEdge.create(this._tooltips.divTT, d, event);
    }
  }
  ...
  _onMouseOutEdge(event, d) {
    ...
    this._graphElem.nodes.classed('NE-HighLight', false);
    if (d.source.cluster && d.target.cluster) {
      if (this._tooltips.clusterEdge != null) this._tooltips.clusterEdge.remove();
    } else {
      if (this._tooltips.normalEdge != null) this._tooltips.normalEdge.remove();
    }
  }
  ...
  _adjustGraph(data) {
    ...
    result.nodes = data.nodes.dataNodes.filter(function (d) {
      return d.visible;
    });
    result.nodes.forEach(function (d, i) {
      d.newIndex = i;
    });
    result.edges = data.edges.dataEdges.filter(function (d) {
      return d.visible;
    });
  ...
  @Method()
  async setPanel(_) {
    if (!arguments.length) return this._nodeEdgePanel;
  ...
  @Method()
  async setLegend(_) {
    if (!arguments.length) return this._nodeEdgeLegend;
  ...
  async setData(_, globalData) {
    ...
    if (this.model.data.isCluster === undefined) {
      vNodesTemp = range(0, this.model.data.nodes.dataNodes.length).map(function () {
        return 0;
      });
      ...
      this.model.data.nodes.dataNodes.forEach(function (d, i) {
        d.grau = vNodesTemp[i];
      });
    }

    this._colorScale = scaleOrdinal(schemeCategory10).domain(await this.getColorBreaks());
    ...
    maxQtNodes = max(this._graphData.nodes, function (d) {
      return d.qtNodes;
    });
    ...
    maxQtEdges = max(this._graphData.edges, function (d) {
      return d.qt;
    });
    if (maxQtEdges === 1) {
    ...
    this._forceLayout
      ...
      .force(
        'forceX',
        forceX()
          .strength(this._configLayout.gravity)
          .x(this.width * 0.7),
      )
      .force(
        'forceY',
    ...
    this._forceLayout
      .on('tick', () => {
        if (this.model.data !== null) var ticksPerRender = this.model.data.nodes.dataNodes.length / 100;
        else var ticksPerRender = 1;
        if (this._graphElem.edges != null) {
          requestAnimationFrame(() => {
            ...
            if (this._forceLayout.alpha() < 0.03) this._forceLayout.stop();
          });
        }
      })
      .on('end', function () {});
  ...
  async indexAttrSize(_) {
    if (!arguments.length) return this._indexAttrSize + 1000;
  ...
  @Method()
  resetHighSearch() {
    this._graphElem.nodes.classed('NE-HighSearch', function (d) {
      return false;
    });
  }
  ...
  componentDidLoad() {
    let svg = select(this.element.querySelectorAll('.nodelink')[0]).attr('width', this.width).attr('height', this.height);
