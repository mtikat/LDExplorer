...
function setDetailsPanel(d){
    ...
    processQuery(queryValues, getRuleId(d))    
}


function setPubContent(result, values){
    const publications = prepareContent(result)

    d3.select('div#loading').remove()
    d3.select('div#'+values.id+'_'+activeChart).select('div.details-content').node().innerHTML += getContent(publications)
}
