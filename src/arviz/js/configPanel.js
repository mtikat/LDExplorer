...
function setColors(){
    ...
    const colors = [ "#e8e8e8", "#b2dede", "#5ac8c8",
...
function setFilteringCriteria(){
    ...
    filtering = {'no_clustering': true,
                'clusters_articles': clusters.filter(d => d.type == 'article'),
                'clusters_terms': clusters.filter(d => d.type == 'label'),
                'clusters_both': clusters.filter(d => d.type == 'label_article'),
                'terms': terms,
                'symmetry': true,
                'no_symmetry': true,
...
function setFilterPanel(){
    ...
    function updateInputRange(){
        ...
        group.select('text#'+range+'-text').text(a.value+' - '+b.value)
...
let width = div.node().clientWidth,
...
// change dataset panel
// setUploadFilePanel()

let filteredData = getFilteredData(); // this variable keeps the filtered data used in both charts, commonly updated
